...
class UserCommunicator(Communicator):
    ...
    def get(self, /, num: int = 1, *, all: bool = False, retain: bool = ..., strict: bool = False) -> List[PacketItem]:
        if all and all is not ...:
            num = len(self._buffer)
            if retain and retain is ...:
                retain = True
            else:
                # TODO Add warning for purging self._buffer when !strict
                if strict:
                    raise CommunicatorPurgeBuffer(errorHandle=ProgrammerErrorHandle("Try not to set all=True and retain=False else you will purge (delete the whole of) the self._buffer!"))
