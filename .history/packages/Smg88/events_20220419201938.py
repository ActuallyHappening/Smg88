...
class EventStage():
    ...
    def subscribe(self, /, callback: Callable = ..., *, channel: str = ...) -> None:
      ...
      Args:
          callback (Callable, optional): Callable to call with event=event when event is posted on that channel. Defaults to ....
          channel (str, optional): The exact channel to . Defaults to ....
      """
        # TODO add some info for this function as it is very useful
      if name is ...:
          name = callback.__name__
        print(
            f"Subscribing to EventStage {callback=} under {name=}")
        self._subscriptions[nam] = callback
